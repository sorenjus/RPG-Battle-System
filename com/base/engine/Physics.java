...
public class Physics {
     ...
     * GameObject
     *
     * @param detectionBox The collision detection box
     ...
     */
    public static GameObject areColliding(final Rectangle detectionBox, final GameObject gameObject) {
        ...
        if(detectionBox.intersects(hitbox)) {
            return gameObject;
        } else {
            return null;
        }
     ...
     * @param secobdyCoord The ending y coordinate
     * @return The calculated distance between (x1, y1) and (x2, y2)
     */
    public static float getDist(final float firstxCoord, final float firstyCoord, final float secondxCoord, final float secobdyCoord) {
        return (float)Math.sqrt(((secondxCoord - firstxCoord) * (secondxCoord - firstxCoord)) + ((secobdyCoord - firstyCoord) * (secobdyCoord - firstyCoord)));
    }
}
